...
- Если вы пользуетесь **VPN** сервисом для доступа к конкретному url ресурсу в Интернете через протоколы `HTTP` или `HTTPS` на своей машине, вы можете предоставить к нему доступ другим машинам в сети, без необходимости устанавливать и использовать VPN на других клиентах.
...
```shell
sudo curl -s -L https://github.com/Lifailon/froxy/releases/download/0.0.2/froxy-0.3.0-linux-x64 -o /usr/local/bin/froxy
...
Пример ssh подключения через прокси сервер (реализация ssh-туннелирования):
...
Пример переадресации запросов от клиента (конфигурация `rsyslog` клиента справа) к [серверу Syslog](https://github.com/MaxBelkov/visualsyslog) (слушающего запросы на порту `514`) через прокси сервер.
