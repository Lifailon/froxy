...
Кроссплатформенная утилита командной строки для реализации классического (прямого) и обратного прокси сервера на базе **.NET**. Поддерживается переадресация любого **HTTPS** трафика (`CONNECT` запросы) для прямого проксирования и **TCP**, **UDP** или **HTTP/HTTPS** протоколы для реализации обратоного прокси. Для переадресации веб-траффика поддерживаются `GET` и `POST` запросы с передачей заголовков и тела запроса от клиента, что позволяет использовать `REST API` запросы и проходить авторизацию на сайтах.
...
Пример запуска классического прокси сервера на порту `8080` с использованием авторизации в контейнере:
...
Если вы планируете использовать обратный прокси сервер, установите значение `FORWARD=0`, вместо этого перейдайте значения в переменные `LOCAL` и `REMOTE`. Если не требуется авторизация, то передайте значение `false` в параметры `USER` и `PASSWORD`.
