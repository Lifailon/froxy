...
- [Установка](#-установка)
- [Сборка](#-сборка)
- [Запуск в контейнере Docker](#-docker)
...
Для предварительной сборки приложения (если этого не было сделано ранее в системе) используйте [dockerfile-build](dockerfile-build):

```shell
docker build -t froxy -f dockerfile-pre-build .
```

Пример запуска классического прокси сервера на порту `8080`:
...
Если вы планируете использовать обратный прокси сервер, оставьте `FORWARD` пустым, вместо этого перейдайте значения в переменные `LOCAL` и `REMOTE`.
