...
- Во первых, если вы используете VPN сервер в режиме точка-точка, и хотите предоставить доступ второй (удаленно подключенной) машине к приложению на другой машине в вашей сети работающих через протокол `TCP`, например, для протоколов `RTSP`, `SSH`, `RDP` или любой другой. Это также выступает альтернативой классического ssh туннелирования, например, через `OpenSSH`, `Putty` или `MobaSSHTunnel`.
